...
import EmptyBuyNowIcon from '@/components/icons/empty-cart';
import { CloseIcon } from '@/components/icons/close-icon';
import BuyNowItem from '@/components/buynow/buynow-item';
    ...
    <section className="flex flex-col h-full relative">
      <header className="fixed max-w-md w-full top-0 z-10 bg-light py-4 px-6 flex items-center justify-between border-b border-border-200 border-opacity-75">
        <div className="flex text-accent font-semibold">
          <BuyNowCheckBagIcon className="shrink-0" width={24} height={22} />
      ...
      {/* End of buynow header */}

      <AnimateSharedLayout>
        <motion.div layout className="flex-grow pt-16 pb-20">
          {items.length > 0 ? (
            items?.map((item) => <BuyNowItem item={item} key={item.id} />)
          ) : (
            <motion.div
              layout
              initial="from"
            ...
            >
              <EmptyBuyNowIcon width={140} height={176} />
...
export default BuyNowSidebarView;
