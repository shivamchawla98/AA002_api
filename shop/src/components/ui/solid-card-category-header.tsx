  ...
  return (
    <Link href={`/${item?.slug}/search/?category=${item.slug}&type=${item?.slug}`}>
      <div className="relative overflow-hidden text-center cursor-pointer group">
        ...
        <span className="block mt-2 text-base font-semibold transition-colors text-heading group-hover:text-orange-500 ltr:text-left rtl:text-right" style={{fontSize: "12px", textAlign: "center"}}>
          {item.name}
        </span>
      </div>
    </Link>
  ...
  return (
    <div className="relative">
      <Swiper
        ...
        slidesPerView={7}
        style={{textAlign: "center", width: "80%", marginLeft: "400px"}}
      >
        {items?.map((category: any, idx: number) => (
          <SwiperSlide key={idx} style={{margin: 0, width: "33%"}}>
