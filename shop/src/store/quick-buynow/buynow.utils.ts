...
export function addItemWithQuantity(
  ...
  if (quantity <= 0)
    throw new Error("buynowQuantity can't be zero or less than zero");
...
export function buynowWithQuantity(
  items: Item[],
  item: Item,
  quantity: number
) {
  if (quantity <= 0)
    throw new Error("Quantity can't be zero or less than zero");
  const existingItemIndex = items.findIndex(
    (existingItem) => existingItem.id === item.id
  );
